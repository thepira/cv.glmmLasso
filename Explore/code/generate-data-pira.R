...
# figuring out that coef = intercept + regressor
fix <- m3 %>% fixef() 
ran <- m3 %>% ranef() 
ran_df_long <- ran %>% as.data.frame()
ran_df_wide <- ran_df_long %>% tidyr::spread(key = term, value = condval)
ran_df_wide %>% mutate(tot_intercept = ran_df_wide$'(Intercept)' + fix[1])
ran_df_wide %>% mutate(tot_experience = ran_df_wide$experience + fix[2])
m3 %>% coef()
